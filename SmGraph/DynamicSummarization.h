...
public:
    ...
    static double getCost(Graph& sg, Graph& origin, int id1, int id2)
    {
    ...
    static void addEdge(Graph& sg, Graph& origin, int src, int trg)
    {
        ...
        if (stype == 1 && ttype == 1)
        {
            ...
            if (srcParticleNode->getParent() == trgParticleNode->getParent())
            {
                srcParticleNode->addCorrectionTarget('+', trg);
                trgParticleNode->addCorrectionTarget('+', src);
            }
            else if (srcParticleNode->getParent() == src && trgParticleNode->getParent() == trg)
            {
